      ...
      >
        <div className="container mx-auto px-6 h-20 flex justify-between items-center">
          ...
          <nav className="hidden md:flex items-center gap-8">
            {navLinks.map((link) => (
              <a key={link.name} href={link.href} onClick={handleSmoothScroll} className="text-white-300 text-white transition-colors relative group">
                {link.name}
              ...
              <nav className="grow flex flex-col justify-center items-center gap-8">
                {navLinks.map((link, index) => (
                  <motion.a
                    ...
                    className="text-3xl font-semibold text-gray-300 hover:text-accent-clearBlue transition-colors"
